...
export default function BulkEmailComposer({ allMembers }) {
  ...
  // Membres actifs avec email uniquement (allMembers est déjà filtré sur status='validated')
  ...
  // Rôles uniques (hors bureau géré séparément)
  const roles = useMemo(() => {
    const set = new Set();
  ...
  // Membres après filtres promo / rôle / recherche
  const filteredMembers = useMemo(() => {
    return emailMembers.filter(m => {
      ...
      if (memberSearch) {
        ...
        const hay = `${m.nom || ""} ${m.email || ""}`.toLowerCase();
  ...
  // Destinataires effectifs (tous filtrés OU sélection manuelle)
  ...
  // Fermeture de la prévisualisation sur Echap
  useEffect(() => {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Envoyez un email personnalisé à tout ou partie des membres actifs. Limite Brevo : 300 emails/jour.
        </p>
      </div>

      {/* Onglets Composer / Historique */}
        ...
        <div className="space-y-3">
          ...
          {logsLoading ? (
            ...
            <div className="bg-card border border-border rounded-2xl p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">Aucune campagne envoyée pour l'instant.</p>
            </div>
          ) : (
                ...
                <div key={log.id}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {log.status === "error" && log.error_message && (
                      <p className="text-xs text-red-400 mt-1 line-clamp-2">{log.error_message}</p>
                    )}
                  </div>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full flex-shrink-0 ${
                    ...
                    {log.status === "success" ? "Envoyé" : "Erreur"}
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            ...
            <div className="p-4 space-y-3">
              ...
              {/* Toggle Tous / Sélection manuelle */}
              ...
              {/* Liste avec checkboxes (sélection manuelle uniquement) */}
              {!selectAll && (
              ...
              {recipients.length > 250 && (
                <div className="flex items-start gap-2 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2.5">
                  ...
                  <p className="text-xs text-amber-300 leading-relaxed">
                    Attention : avec {recipients.length} destinataires, vous approchez de la limite journalière Brevo (300 emails/jour sur le plan gratuit).
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="space-y-4">
              ...
              <div className="p-4 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1.5">
                    ...
                    {" "}pour insérer le prénom de chaque destinataire.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <button
                onClick={() => { if (validateForm()) setPreviewOpen(true); }}
          ...
          <div className="bg-background border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden"
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-border flex-shrink-0">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  ...
                  {" · "}{"{{prenom}}"} → « {samplePrenom} » (1er destinataire)
                </p>
              </div>
            ...
            <div className="flex gap-3 px-5 py-4 border-t border-border flex-shrink-0">
              ...
              <button onClick={handleSend} disabled={sending}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                {sending
