      ...
      <AnimatePresence>
        {isCartOpen && (
          ...
          >
                ...
                <AnimatePresence>
                  {items.map(item => (
                    <motion.div
                      ...
                      style={{ background: "rgba(52,211,153,0.06)", border: "1px solid rgba(52,211,153,0.10)" }}
                    >
                      <div className="w-12 h-12 rounded-lg flex-shrink-0 flex items-center justify-center text-2xl select-none"
                        style={{ background: item.imgBg || "rgba(52,211,153,0.12)" }}>
                        {item.emoji}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-xs font-semibold leading-tight line-clamp-2">{item.name}</p>
                        <p className="text-xs font-bold mt-0.5" style={{ color: "#6ee7b7" }}>{fmt(item.price)}</p>
            ...
            {/* Footer */}
            {items.length > 0 && (
              <div className="px-5 py-4" style={{ borderTop: "1px solid rgba(52,211,153,0.10)" }}>
