...
import { Plus, Trash2, Link2, BarChart2, Eye, EyeOff, Loader2, X } from "lucide-react";
...
const emptyForm = {
  titre: "", description: "", optionsRaw: "Option A\nOption B\nOption C",
...
export default function SondagesSection() {
  ...
  const [form, setForm] = useState(null); // null = fermé, {} = nouveau
  const [expanded, setExpanded] = useState({}); // id → bool résultats visibles
      ...
      {form && (
        <div className="bg-white border border-border rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="p-5 grid md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <Field label="Question / Titre *">
                <input className={inp} value={form.titre} onChange={e => setForm(p => ({ ...p, titre: e.target.value }))} placeholder="Ex : Quel lieu préférez-vous pour l'AG ?" />
              </Field>
            </div>
            <div className="md:col-span-2">
              <Field label="Description (optionnelle)">
                <textarea className={inp} rows={2} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Contexte ou précisions…" />
              </Field>
            </div>
            <div className="md:col-span-2">
              <Field label="Options (une par ligne, minimum 2)">
                <textarea className={inp} rows={5} value={form.optionsRaw} onChange={e => setForm(p => ({ ...p, optionsRaw: e.target.value }))} placeholder={"Option A\nOption B\nOption C"} />
              </Field>
            </div>
            <Field label="Date de clôture (optionnelle)">
              <input type="date" className={inp} value={form.expires_at} onChange={e => setForm(p => ({ ...p, expires_at: e.target.value }))} />
            </Field>
            <div className="flex flex-col gap-3 pt-1">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input type="checkbox" checked={form.multiple_choix} onChange={e => setForm(p => ({ ...p, multiple_choix: e.target.checked }))} className="w-4 h-4 rounded accent-primary" />
          ...
          <div className="flex justify-end gap-2 px-5 pb-5">
            <button onClick={() => setForm(null)} className="px-4 py-2 text-sm font-medium text-muted-foreground border border-border rounded-xl hover:bg-muted transition-colors">Annuler</button>
