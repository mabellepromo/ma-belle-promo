...
const MULTI_YEARS = Array.from({ length: 5 }, (_, i) => new Date().getFullYear() - 4 + i);
...
export default function CotisationsSection({ members }) {
  ...
  const [vue,            setVue]            = useState("annuelle"); // "annuelle" | "historique"
  ...
  const { data: multiData, loading: multiLoading } = useMultiYearCotisations(MULTI_YEARS);
      ...
      <div className="flex flex-wrap items-start gap-3">
        ...
        <div className="ml-auto flex items-center gap-2 flex-wrap">
          ...
          <div className="flex rounded-xl border border-border overflow-hidden">
            <button onClick={() => setVue("annuelle")}
              className={`px-3 h-9 text-sm font-medium transition-colors ${vue === "annuelle" ? "bg-primary text-primary-foreground" : "bg-background text-muted-foreground hover:bg-muted"}`}>
              Par année
            </button>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/20">
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-muted-foreground w-48">Membre</th>
                  {MULTI_YEARS.map(y => (
                    <th key={y} className={`text-center px-3 py-2.5 text-xs font-semibold ${y === currentYear ? "text-primary" : "text-muted-foreground"}`}>{y}</th>
                  ))}
              ...
              <tbody className="divide-y divide-border/50">
                {(members ?? []).map(m => {
                  const memberData = multiData[String(m.id)] ?? {};
                  const paye_count = MULTI_YEARS.filter(y => memberData[y]?.statut === "payé").length;
                  return (
                    <tr key={m.id} className="hover:bg-muted/10">
                      <td className="px-4 py-2.5">
                        <p className="font-semibold text-foreground text-xs truncate max-w-[180px]">{m.nom}</p>
                        {m.bureau && <span className="text-[9px] text-primary font-bold uppercase">Bureau</span>}
                      </td>
                      {MULTI_YEARS.map(y => {
                        const cot = memberData[y];
                        const cfg = cot ? CELL_CONFIG[cot.statut] : null;
                        return (
                          <td key={y} className="px-3 py-2.5 text-center">
                            {cfg ? (
                              <span className={`inline-flex items-center justify-center w-7 h-7 rounded-lg text-white text-xs font-bold ${cfg.bg}`} title={cot.statut}>
                                {cfg.label}
                              </span>
                            ) : (
                              <span className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-muted/40 text-muted-foreground text-xs">—</span>
                            )}
                          </td>
                        );
                      })}
